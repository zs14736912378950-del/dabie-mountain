...
function verifyPuzzle(container) {
  ...
  if (allCorrect && cardsPlacedCount >= totalSlots) {
    playSuccess();
    showToast(container, "🎉 太棒了！完整食物链拼图完成！");
    unlockChapter(4);
    gsap.to(slotEls, { scale: 1.05, duration: 0.2, yoyo: true, repeat: 2, ease: "power2.inOut",
      onComplete: () => { setTimeout(() => navigateTo("crisis"), 1500); }
    });
  } else {
    playError();
