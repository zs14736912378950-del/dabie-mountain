...
import { getSpeciesIllustration } from "../components/speciesIllustrations.js";
import { showBannerToast } from "../utils/ui.js";
...
export function init(container) {
  ...
  _onDragMove = function(e) {
    ...
    if (dt > 0) {
      _lastMoveX = (cx - _lastMoveX) / dt * 16; // velocity per frame
    }
    _lastMoveX = cx;
    _lastMoveTime = now;
  ...
  _onDragEnd = function() {
    ...
    // 惯性滚动
    var velocity = _lastMoveX * 8;
    if (Math.abs(velocity) > 5) {
      var targetOffset = Math.max(-maxOffset, Math.min(0, offsetX + velocity));
      gsap.to({v: offsetX}, {
        v: targetOffset,
        duration: 0.8,
        ease: "power1.out",
        onUpdate: function() {
          var cur = this.targets()[0].v;
          layerEls.forEach(function(el,i){el.style.transform="translateX("+(cur*parallaxSpeeds[i])+"px)";});
          if(hotspotsEl)hotspotsEl.style.transform="translateX("+(cur*parallaxSpeeds[3])+"px)";
        },
        onComplete: function() { offsetX = targetOffset; }
      });
    }
    _lastMoveX = 0; _lastMoveTime = 0;
  };
  ...
  scene.querySelectorAll(".hotspot").forEach(function(hot){
    hot.addEventListener("pointerup",function(e){
      ...
      if(!discoveredSpecies.has(idx)){
        ...
        gsap.fromTo(hot, {scale: 1.3, rotate: 5}, {scale: 1, rotate: 0, duration: 0.3, ease: "back.out(1.4)"});
        setTimeout(function() {
          showBannerToast("\ud83c\udf1f \u53d1\u73b0\u65b0\u7269\u79cd\uff1a" + sp.name + "\uff01");
        }, 200);
        setTimeout(function() {
          showKnowledgeCard(container, sp, hot);
        }, 400);
      }else{
        playClick();
      ...
      if(discoveredSpecies.size>=totalSpecies){setTimeout(function(){playCelebration();showBannerToast("\ud83c\udf89 \u5168\u90e8\u7269\u79cd\u53d1\u73b0\u5b8c\u6210\uff01\u4f60\u662f\u68ee\u6797\u63a2\u7d22\u5bb6\uff01");spawnExploreParticles(container);},600);}
    });
  });
  ...
  scene.addEventListener("click", function(e) {
    if (dragging || _squirrelFound) return;
    if (e.target.closest(".hotspot") || e.target.closest(".knowledge-card")) return;
    var rect = scene.getBoundingClientRect();
    var x = e.clientX - rect.left + Math.abs(offsetX);
    // 大树位置：x≈800 或 x≈2200（场景坐标）
    var treeHit = (Math.abs(x - 800) < 80 || Math.abs(x - 2200) < 80);
    if (treeHit) {
      _treeClickCount++;
      clearTimeout(_treeClickTimer);
      _treeClickTimer = setTimeout(function() { _treeClickCount = 0; }, 2000);
      if (_treeClickCount >= 5) {
        _squirrelFound = true;
        spawnSquirrel(container, x > 1500 ? 2200 : 800);
      }
    }
  });
...
// 彩蛋：小松鼠
function spawnSquirrel(container, treeX) {
  var scene = container.querySelector("#explore-scene");
  if (!scene) return;

  var squirrel = document.createElement("div");
  squirrel.textContent = "🐿️";
  squirrel.style.cssText = "position:absolute;font-size:28px;z-index:20;pointer-events:none;";
  // 树的坐标转换为百分比
  var pctX = (treeX / 3400) * 100;
  squirrel.style.left = pctX + "%";
  squirrel.style.top = "40%";
  scene.appendChild(squirrel);

  // 松鼠动画：从树上探出头
  gsap.fromTo(squirrel, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, ease: "back.out(1.4)" });

  // 显示提示
  showBannerToast("🐿️ 你发现了一只隐藏的小松鼠！观察力真棒！");

  // 5秒后消失
  setTimeout(function() {
    gsap.to(squirrel, { y: -20, opacity: 0, duration: 0.5, onComplete: function() { squirrel.remove(); } });
  }, 5000);
}
