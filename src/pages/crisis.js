...
let ecoScore = 0;
let choicesMade = [];
let consecutiveCorrect = 0; // 连续正确计数
...
function showResult(container, scenario, choice, choiceIdx) {
  ...
  var severeWarning = "";
  if (choice.score <= -8) {
    severeWarning = '<p style="color:var(--color-danger);font-size:13px;text-align:center;margin-top:8px;font-weight:600;">⚠️ 这个选择对生态系统造成了难以挽回的伤害……</p>';
  }
  ...
  if (choice.score > 0) {
    ...
    if (consecutiveCorrect >= 3) {
      ...
      showGuide("crisis_congrats", container, {
        dismiss: function() {}
      });
    }
  } else {
  ...
  // 错误选择：场景图片变灰效果
  if (choice.score < 0) {
    const sceneWrap = main.querySelector(".crisis-scene-wrap");
    if (sceneWrap) {
      gsap.to(sceneWrap, {
        ...
        onComplete: function() {
          // 1.2秒后恢复
          gsap.to(sceneWrap, { filter: "grayscale(0)", opacity: 1, duration: 0.6, delay: 1.2, ease: "power2.out" });
        }
      });
    }
  }
  ...
  // 分数翻滚动画
  var scoreTextEl = main.querySelector(".eco-score-text");
  if (scoreTextEl) {
    var startVal = ecoScore - choice.score;
    var endVal = ecoScore;
    ...
    gsap.to(obj, {
      ...
      ease: "power2.out",
      onUpdate: function() {
        scoreTextEl.textContent = "生态值: " + Math.round(obj.val);
      }
    });
  }
  ...
  if (nextBtn) {
    nextBtn.addEventListener("pointerup", (e) => {
      ...
      if (currentScenario >= scenarios.length) {
        // 全部危机结束：青铜级显示鼓励语
        if (ecoScore < 30) {
          showGuide("crisis_bronze", container, {
            continue: function() {
              playCelebration();
              unlockChapter(5);
...
function spawnGoldenParticles() {
  ...
  for (var i = 0; i < 12; i++) {
    setTimeout(function() {
      ...
      el.style.cssText = "position:fixed;left:50%;top:50%;font-size:" + (16 + Math.random() * 12)
        + "px;pointer-events:none;z-index:2000;";
